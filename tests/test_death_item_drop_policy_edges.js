...
function run() {
    ...
    // Upstream's death classification resolves context.killer ahead of the raw
    // damage source, so a player-owned servitor kill is player-caused for both
    // the EXP penalty and the item-drop rules.
    cause = die.resolveDeathCause({ source: summon, killer: player }, null);
    assert.strictEqual(cause.killerPlayable, true,
        'the resolved owner outranks the servitor damage source');
    assert.strictEqual(cause.playerControlledKiller, true,
    ...
    // Upstream removed the session-actor fallback: an unattributed death is not
    // player-caused, and a self-inflicted death never is.
    cause = die.resolveDeathCause({}, player);
    assert.strictEqual(cause.killerPlayable, false,
        'an unattributed death has no playable cause');
    assert.strictEqual(cause.playerControlledKiller, false);

    cause = die.resolveDeathCause({ killer: player }, player);
    assert.strictEqual(cause.killerPlayable, false,
        'a self-inflicted death is never player-caused');
    assert.strictEqual(cause.playerControlledKiller, false);
