...
const medusa = skill(367, 'Dance of Medusa');
const medusaRule = medusa.fetchSemantic();
assert.strictEqual(medusa.fetchTargetKind(), 'enemy');
assert.strictEqual(medusaRule.sourceTarget, 'aura');
assert.strictEqual(medusaRule.radius, 150);
assert.strictEqual(medusaRule.isDance, false, 'Medusa is an offensive skill, not a stacking party dance');
assert.strictEqual(attack.skillUseConditionFailure(effectsActor(), medusa), 'Incorrect weapon.');
assert.strictEqual(attack.skillUseConditionFailure(dancerWithDuals, medusa), null);
const stoneTarget = { effects: {}, fetchId: () => 100001, fetchLevel: () => 77 };
const medusaCaster = { effects: {}, fetchId: () => 2000001, fetchLevel: () => 77 };
medusaCaster.fetchLocX = () => 0;
medusaCaster.fetchLocY = () => 0;
const nearbyEnemy = { fetchId: () => 100003, fetchAttackable: () => true, fetchLocX: () => 150 };
const distantEnemy = { fetchId: () => 100004, fetchAttackable: () => true, fetchLocX: () => 151 };
const friendlyNpc = { fetchId: () => 100005, fetchAttackable: () => false };
const areaAttack = new Attack();
areaAttack.fetchSkillTargetsInRadius = () => [medusaCaster, nearbyEnemy, distantEnemy, friendlyNpc];
assert.deepStrictEqual(areaAttack.resolveSkillTargets(null, medusaCaster, medusaCaster, medusa), [nearbyEnemy], 'Medusa must hit only enemies within its caster-centered radius');
const outcome = invoke('GameServer/Skills/C4SkillEffects').execute(null, medusaCaster, stoneTarget, medusa, { rng: () => 0 });
assert(outcome.effect, 'Successful Medusa must petrify its target');
assert.strictEqual(medusaRule.durationMs, 30000);
assert(outcome.effect.expiresAt - Date.now() > 29000);
const restrictions = invoke('GameServer/Effects/EffectRestrictions');
assert.strictEqual(restrictions.canMove(stoneTarget), false);
assert.strictEqual(restrictions.canAttack(stoneTarget), false);
assert.strictEqual(restrictions.canCast(stoneTarget), false);
assert.strictEqual(restrictions.canMove(medusaCaster), true);
assert.strictEqual(EffectStore.list(medusaCaster).length, 0, 'Caster must not receive a Medusa buff');
assert.strictEqual(EffectStore.abnormalMask(stoneTarget), 0x0800);
stoneTarget.setHp = () => { throw new Error('Petrified target must not take damage'); };
invoke('GameServer/Actor/Generics/ReceivedHit')(null, stoneTarget, 100);
invoke('GameServer/Npc/Generics/ReceivedHit')(null, medusaCaster, stoneTarget, 100);
outcome.effect.expiresAt = Date.now() - 1;
assert.strictEqual(restrictions.canMove(stoneTarget), true);
assert.strictEqual(EffectStore.abnormalMask(stoneTarget), 0);
const resistedStone = { effects: {}, fetchId: () => 100002, fetchLevel: () => 77 };
invoke('GameServer/Skills/C4SkillEffects').execute(null, medusaCaster, resistedStone, medusa, { rng: () => 1 });
assert.strictEqual(EffectStore.list(resistedStone).length, 0);
