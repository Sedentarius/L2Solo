...
// Availability is asked of the cache the running server actually consults, not
// of a hand-picked subset of datapack files: a quest may only reach VERIFIED
// when its templates and spawns resolve exactly as they will in play.
...
for (const group of DataCache.npcSpawns) {
  for (const spawn of group.spawns || []) spawnedIds.add(Number(spawn.selfId));
}
