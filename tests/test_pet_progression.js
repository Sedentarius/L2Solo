...
async function main() {
    ...
    const journal = () => Q.active(session).find(quest => quest.id === 420)?.condition;
    ...
    for (let n=0;n<LittleWing.dragons.length;n++) {
        ...
        assert.strictEqual(journal(),2,'crafted stone must not send the player to Byron before Cronos');
        const savedCond = state(420).getInt('cond');
        session.questStatesLoaded = false;
        await Q.ensureLoaded(session);
        assert.strictEqual(state(420).getInt('cond'),savedCond,'internal saved progress is unchanged');
        assert.strictEqual(journal(),2,'journal mapping survives reload');
        await event(420,7610,'cronos');
        assert.strictEqual(journal(),3,'Cronos sends the player to Byron');
        await event(420,7711,'byron');
        assert.strictEqual(journal(),4,'Byron sends the player to the fairy');
        await event(420,7747,'fairy');
        assert.strictEqual(journal(),5,'the fairy sends the player to a dragon');
        assert.strictEqual(count(3821),1);
        await event(420,dragon.npc,`dragon_${dragon.npc}`);
        assert.strictEqual(journal(),6,'the dragon starts the egg collection stage');
        await event(420,dragon.npc,`eggs_${dragon.npc}`);
        assert.strictEqual(journal(),6,'empty-handed hand-in cannot advance the journal');
        ...
        assert.strictEqual(journal(),6,'collected eggs must still be returned to the dragon');
        await event(420,dragon.npc,`eggs_${dragon.npc}`);
        assert.strictEqual(count(dragon.egg),1,'twenty rescued eggs become one gifted egg');
        assert.strictEqual(journal(),7,'only the gifted egg sends the player back to the fairy');
