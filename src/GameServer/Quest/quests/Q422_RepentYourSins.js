...
// it. See docs/c4/quests/runtime-sources.md.
//
...
module.exports = {
    ...
    async onEvent(state, event) {
        ...
        if (event === 'reissue') {
            ...
            if (state.isStarted() || !count(state, LEFTOVER_MANACLES)) return null;
            await step(state, {
        ...
        if (event === 'repent') {
            ...
            actor.setPk(cleared ? 0 : carried - removed);
            await invoke('Database').updateCharacterPvpPkKarma(actor.fetchId(),
                actor.fetchPvp?.() || 0, actor.fetchPk(), actor.fetchKarma?.() || 0);
