...
// Thirteen quests then gate their beginner-shot grant on that flag plus a
// character-wide NEWBIE_SHOTS_RECEIVED counter:
//
//   Q101-Q108 (one-time class quests)   counter < 2, 7000 soulshots / 3000 spiritshots
//   Q257/260/265/273/293 (repeatable)   counter < 1, 6000 soulshots / 3000 spiritshots
//                                       plus a per-quest receipt so a repeatable
//                                       quest pays the reward at most once.
//
...
function isEligible(actor) {
    ...
    const flag = Number(actor.newbie ?? actor.fetchNewbie?.() ?? UNKNOWN);
...
function plan(actor, { threshold, soulshots, spiritshots = 0, orcUsesSoulshots = false }) {
    ...
    const received = Math.max(0, Number(actor.newbieShotsReceived ?? actor.fetchNewbieShotsReceived?.() ?? 0));
