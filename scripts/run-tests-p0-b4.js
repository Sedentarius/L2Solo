...
for (const test of tests) {
    ...
    if (result.status !== 0) {
        failures += 1;
        console.error(`P0-B4 failure: ${test} (exit ${result.status})`);
    }
}

if (failures) {
    console.error(`\nP0-B4 focused regression pack failed: ${failures}/${tests.length} test(s).`);
    process.exitCode = 1;
} else {
    console.log(`\nP0-B4 focused regression pack passed (${tests.length} tests).`);
}
